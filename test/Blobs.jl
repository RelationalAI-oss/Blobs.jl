...
@testset "BlobString{$N}" for N in (8, 16, 64, 1024, 1024^2)

    BS = BlobString{N}

    @test Blobs.self_size(BS) == 16

    data = Blob{UInt8}(Libc.malloc(8), 0, 8)
    @test_nowarn BS(data, 8)[8]
    # pretty much any access to a unicode string touches beginning and end
    @test_throws BoundsError BS(data, 16)[8]
    ...
    s = "普通话/普通話"
    bbs = Blobs.malloc_and_init(BS, s)
    bs = bbs[]
